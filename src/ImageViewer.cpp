...
void ImageViewer::render3D()
{
	...
	int shadingType = ui->rbConstantShading->isChecked() ? 0 : 1;
	...
	int ro = ui->sbAmbientR->value();
	int go = ui->sbAmbientG->value();
	int bo = ui->sbAmbientB->value();
	lightSource.Io = QVector3D(ro / 255.0, go / 255.0, bo / 255.0);
	int rl = ui->sbLightR->value();
	int gl = ui->sbLightG->value();
	int bl = ui->sbLightB->value();
	lightSource.Il = QVector3D(rl / 255.0, gl / 255.0, bl / 255.0);
	vW->setLight(lightSource);

	Material material;
	double rsR = ui->dsbReflectionR->value();
	double rsG = ui->dsbReflectionG->value();
	double rsB = ui->dsbReflectionB->value();
	material.rs = QVector3D(rsR, rsG, rsB);
	double rdR = ui->dsbDifusionR->value();
	double rdG = ui->dsbDifusionG->value();
	double rdB = ui->dsbDifusionB->value();
	material.rd = QVector3D(rdR, rdG, rdB);
	double raR = ui->dsbAmbientR->value();
